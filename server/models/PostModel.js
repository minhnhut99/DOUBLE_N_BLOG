...
const PostModel = {
    getListPost: async () => {
        const strQuery = `SELECT p.p_id, p_title, p_desc, p_view, 
    ...
    getDetailPost: async (id) => {
        const strQuery = `SELECT p.p_id, p_title, p_desc, p_content, p_view, p_status, c_name, p.u_id, u_name, p.created_at,
    ...
    createPosts: async (data) => {
        const strQuery = `
            INSERT INTO posts(p_title, p_desc, p_content, p_status, u_id, c_id)
            VALUES ('${data.title}', '${data.desc}', '${data.content}', 0, ${data.userId}, ${data.cateId})
        `;
        const result = await query(strQuery);
        return result;
    },
    updatePosts: async (data) => {
        const strQuery = `
            UPDATE posts
            SET p_title = '${data.title}', p_desc = '${data.desc}', p_content = '${data.content}', c_id = ${data.cateId}
            WHERE p_id = ${data.id}
        `;
        const result = await query(strQuery);
        return result;
